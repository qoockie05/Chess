...
IMAGE_SIZE=64
MODEL_DIR = "lastmodel_21.06.2026"
...
with open(f"{MODEL_DIR}/class_to_idx.json", "w", encoding="utf-8") as f:
    ...
    with torch.no_grad():
        for data, target in loader:
            ...
            if epoch == 20:
                ...
                for id in idx:
                    save_image(data[id],
                f"results/{counter}_rozpoznano_{pred[id].item()}_prawidlowy_{target[id].item()}.png")
                    counter += 1
...
for epoch in range(1, n_epochs + 1):
    ...
    if test_acc > best_acc:
        best_acc = test_acc
        torch.save(network.state_dict(), f"{MODEL_DIR}/best_model.pth")
...
plt.tight_layout()
plt.savefig(f"{MODEL_DIR}/metrics.png")
plt.show()
